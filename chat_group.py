...
class Group:
    ...
    def connect_game(self, me, peer):
        peer_in_group = False
        # if peer is talking or playing, don't join
        peer_in_group, group_key = self.find_group(peer)
        if group_key == S_ALONE:
            print(peer, "is idle as well")
            self.grp_ever += 1
            group_key = self.grp_ever
            self.chat_grps[group_key] = []
